 ...
 */
const isDivisibleBy3 = (num: number): boolean => {
 ...
 */
const isDivisibleBy11 = (num: number): boolean => {
 ...
 */
const processNumbers = (numbers: number[]): string[] => {
    ...
    return numbers.map(num => {
        ...
        if (divisibleBy3 && divisibleBy11) return 'fizzbuzz';
        if (divisibleBy3) return 'fizz';
        if (divisibleBy11) return 'buzz';
        return 'baz';
    });
};
